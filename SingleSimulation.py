...
def simulate(datalabel,anglefactor,radiusfactor,TCRIT): 
	...
	N 			= np.array([20,10],dtype=np.int32)			#N[0] is inner most ring of particles with N[0] particles, etc.
        #velFactor 		= np.asarray([[1.32],[1.32],[0.258],[0.65],[0.35]],dtype=np.float64)	
	...
	tcrit 			= 375#[myr]					# Limited by the 10**10 value used in the algorithm
	...
	NumberOfGalaxies = 2
	GalaxyMassRatios = np.array([1.,1.,2/10,3/10,1/10],dtype=np.float64)
	xangle = np.array([-85*np.pi/180,	10*np.pi/180,		+90.*np.pi/180],dtype=np.float64)#[rad]#Must have length >= NumberOfGalaxies , Angle around the x angle around which to rotate
	yangle = np.array([0*np.pi/180.,	0.*np.pi/180,	0.0],dtype=np.float64)#[rad]#Must have length >= NumberOfGalaxies , Angle around the y angle around which to rotate
	zangle = np.array([-10.*np.pi/180,	0.*np.pi/180,			0*np.pi/180.],dtype=np.float64)#[rad]#Must have length >= NumberOfGalaxies , Angle around the z angle around which to rotate

	#"""#Transformations for playing around with
	r_transform = np.array([[-30.,-25,0.],#first galaxy
				[+30.,+25,0.],#second galaxy
				[12.,-12,75]],#third galaxy
				dtype=np.float64)#[kpc]
	#THIS IS REALLY GOOD, BUT I NEED TO BREAK THE BLUE TAIL UP

	v_transform = np.array([[50.,5.,1.],#first galaxy
				[-125.,-5.,-1.],#second galaxy
