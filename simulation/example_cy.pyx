...
from StartPositions import StartPositions
...
cdef returnMV2d(double[:,:] a):	#Returns a fortran contiguous array from input np.ndarray
	cdef double[:,:] narr_view = np.asfortranarray(a,dtype=np.float64)
	return narr_view
cdef returnMV1d(double[:] a):	#Returns a fortran contiguous array from input np.ndarray
	cdef double[:] narr_view = np.asfortranarray(a,dtype=np.float64)
	return narr_view
	
cdef double sqr(double x) nogil:
	return sqrt(x)

#NBody simulation main Function
cpdef NBody(int[:] NP,double eta, double deltat, double tcrit,double eps2, int BH, double[:,:] kv,double r, double BHM, int stepscrit,double[:,:] r_transform,double[:,:] v_transform,double[:] xangle,double[:] yangle,double[:] zangle,int GalNum,double[:] GalMass,double anglefactor):
	cdef double TESTVALUE = 0
	
	cdef double time = 0.0#[myr]
	cdef double tnext = 0.0
	cdef double Energy = 0.0#[J]
	cdef int nsteps = 0
	cdef int graphCounter = 0
	
	cdef double kmPERpc = 3.085677581e13#[km/pc]
	cdef double kgPERSM = 1.989e30#[kg/SolarMass]
	cdef double sPERmyr = 3.154e13
	cdef double G = 0.004301#[pc*SolMass*(km/s)**2]
	cdef double Gpc = G/(kmPERpc*kmPERpc) #[pc**3*SolMass**-1*s**-2]
	cdef double Gpcmyr = Gpc*sPERmyr**2 #[pc**3*SolMass**-1*myr**-2]
	...
	cdef double[::1,:] x = np.ndarray([3,N],dtype=DTYPE,order='F')      	#r_t		Updated positions
	cdef double[::1,:] x0dot = np.ndarray([3,N],dtype=DTYPE,order='F')	#v_0		Starting velocity
	cdef double[::1,:] x0 = np.ndarray([3,N],dtype=DTYPE,order='F')		#r_0		Starting positions & 4th Order Predicted positions
	cdef double[:] t0 = np.ndarray([N],dtype=DTYPE)      			#t_0		Starting time
	cdef double[:] body = np.ndarray([N],dtype=DTYPE)			#m		Mass
	cdef double[:] step = np.ndarray([N],dtype=DTYPE) 		    	#delt (_i)	Step for each individual particle
	cdef double[::1,:] f = np.ndarray([3,N],dtype=DTYPE,order='F')        	#F		Force
	cdef double[::1,:] fdot = np.ndarray([3,N],dtype=DTYPE,order='F')    	#F^(1)		Force derivative
	cdef double[::1,:] d1 = np.ndarray([3,N],dtype=DTYPE,order='F')       	#D^1	
	cdef double[::1,:] d2 = np.ndarray([3,N],dtype=DTYPE,order='F')      	#D^2
	cdef double[::1,:] d3 = np.ndarray([3,N],dtype=DTYPE,order='F')       	#D^3
	cdef double[:] t1 = np.ndarray([N],dtype=DTYPE)       			#t_1	
	cdef double[:] t2 = np.ndarray([N],dtype=DTYPE)       			#t_2	
	cdef double[:] t3 = np.ndarray([N],dtype=DTYPE)       			#t_3	
	#Not for each individual particle
	cdef double[::1,:] a = np.ndarray([17,N],dtype=DTYPE,order='F')        	#
	cdef double[:] f1 = np.ndarray([3],dtype=DTYPE)        			#	
	cdef double[:] f1dot = np.ndarray([3],dtype=DTYPE)     			#	
	cdef double[:] f2dot = np.ndarray([3],dtype=DTYPE)     			#	
	cdef double[:] f3dot = np.ndarray([3],dtype=DTYPE)    			#	
	
	#Values I added
	cdef double[:] R = np.ndarray([N],dtype=DTYPE,order='F')
	cdef double[:] velocity= np.ndarray([N],dtype=DTYPE)
	cdef double[:] xout = np.zeros([N*8000],dtype=DTYPE)
	cdef double[:] yout = np.zeros([N*8000],dtype=DTYPE)
	cdef double[:] zout = np.zeros([N*8000],dtype=DTYPE)
	cdef double[:] Eout = np.zeros([N*8000],dtype=DTYPE)
	cdef double[:] vout = np.zeros([N*8000],dtype=DTYPE)
	cdef double[:] tout = np.zeros([N*8000],dtype=DTYPE)
	
	#Temporary constants
	cdef double[:] s = np.ndarray([N],dtype=DTYPE)
	cdef double dt = 0
	cdef double dt1 = 0
	cdef double dt2 = 0
	cdef double dt3 = 0
	...
	for i in range(0,N):	#No prange because each thread will overwrite f1dot[k],f2dot[k],f3dot[k]
		for j in range(0,N):
			if j != i:
				for k in range(0,3):
					a[k,i] = x0[k,j] - x0[k,i]
...
#Starting the while loop to move the particles
	...
	cdef np.ndarray x_timed = np.ndarray([N,graphCounter],dtype=DTYPE,order='F')
	cdef np.ndarray y_timed = np.ndarray([N,graphCounter],dtype=DTYPE,order='F')
	cdef np.ndarray z_timed = np.ndarray([N,graphCounter],dtype=DTYPE,order='F')
	cdef np.ndarray	v_timed = np.ndarray([N,graphCounter],dtype=DTYPE,order='F')
	cdef np.ndarray E_arr = np.ndarray([graphCounter-1],dtype=DTYPE)
	cdef np.ndarray step_time = np.ndarray([graphCounter],dtype=DTYPE)
