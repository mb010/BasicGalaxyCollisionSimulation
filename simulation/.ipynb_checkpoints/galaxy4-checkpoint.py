...
velFactor 		= np.asarray([[1.32,1.03],[1.32,1.03],[0.35,.265]],dtype=np.float64)		# velFactor[0] is factor for inner most ring (then indices move outwards along number of rings), must be the same length as N
...
radius 			= 15.#[kpc]					#R/N[-1] is radius factor. RadFactor*N[i]/N[-1] for radius of ring
...
xangle = np.array([0,			0,		60.*np.pi/180],dtype=np.float64)#[rad]#Must have length >= NumberOfGalaxies , Angle around the x angle around which to rotate
yangle = np.array([30*np.pi/180.,	-30.*np.pi/180,	0.*np.pi/180],dtype=np.float64)#[rad]#Must have length >= NumberOfGalaxies , Angle around the y angle around which to rotate
...
r_transform = np.array([[-15/2.,-35/2.,0.],#first galaxy
			[15/2.,35/2.,0.],#second galaxy
			[-80/2,-50/2,40/2]],#third galaxy
			dtype=np.float64)#[kpc]

v_transform = np.array([[90.,5.,0.],#first galaxy
...
#twodview.finalsliderplt(0,2,-5,x_timed,y_timed,z_timed)
#twodview.finalsliderplt(0,2,-5,x_timed,y_timed,z_timed)
